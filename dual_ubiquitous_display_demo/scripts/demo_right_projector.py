...
if __name__ == '__main__':
    rospy.init_node('demo_right_projector_imshow', anonymous=True)

    pts_src = np.array([[0,0], [1919,0], [1919, 1079],[0,1079]],np.float32)
    pts2_org = np.array([[-430, 2200],[1050, 2200],[760, 1040],[-200, 1040]])

    h, status = cv2.findHomography(pts_src, pts2_org)
    h = np.linalg.inv(h)

    new_pts_dst = np.array([[0, 0],[0, 0],[0, 0],[0, 0]],np.float32)

    f = np.float32([[-200.0, 1570.0 + 225.0, 1.0]])
    f = f.T
    f = np.dot(h, f)
    f = f / f[2][0]
    ...
    s = np.float32([[600.0, 1570.0 + 225.0, 1.0]])
    s = s.T
    s = np.dot(h, s)
    s = s / s[2][0]
    ...
    t = np.float32([[-200.0, 1570.0 - 225.0, 1.0]])
    t = t.T
    t = np.dot(h, t)
    t = t / t[2][0]
    ...
    fo = np.float32([[600.0, 1570.0 - 225.0, 1.0]])
    fo = fo.T
    fo = np.dot(h, fo)
    fo = fo / fo[2][0]
    ...
    rospy.set_param("right_projector/file_name", "nav_180.mov")
